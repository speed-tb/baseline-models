...
def prep_ngram_from_text(lang_dir, ngram=2, force=False):
    ...
    os.system(f"change folder path to yours/asr2k/srilm-1.7.3/bin/i686-m64/ngram-count -text {lang_dir / 'train_raw.txt'} -order {ngram} -tolower -write {lang_dir / output_file}")
...
def prep_grammar_from_ngram(lang_dir, ngram=2, force=False):
    ...
    if ngram == 3:
        ...
        os.system(f"change folder path to yours/asr2k/srilm-1.7.3/bin/i686-m64/ngram-count -order 3 -read {lang_dir / 'trigrams.txt'} -lm {lang_dir / 'text3.arpa'}")
        ...
        os.system(f"change folder path to yours/asr2k/srilm-1.7.3/bin/i686-m64/ngram-count -order 2 -read {lang_dir / 'bigrams.txt'} -lm {lang_dir / 'text2.arpa'}")
        ...
        os.system(f"change folder path to yours/asr2k/srilm-1.7.3/bin/i686-m64/ngram-count -order 1 -read {lang_dir / 'unigrams.txt'} -lm {lang_dir / 'text1.arpa'}")
